...
void selectionsort(int a[],int n)//sx chon truc tiep
 {
int min;
for(int i=1;i<n;i++)
	{
...
void insertionsort(int a[],int n)//sx chen truc tiep
{
	int pos;
	int x;
	for(int i=1;i<=n;i++)
...
void interchangeSort(int a[],int n)//sx doi cho truc tiep
{
int i,j;
	for(i=1;i<n-1;i++)
		for(j=i+1;j<n;j++)
...
void Shift (int a[], int left, int right)// hieu chinh day a1,a1+1 .. aN thanh heap
{  
	int  x, curr, joint;
...
void CreateHeap(int a[], int N)// hieu chinh day a1,a2...aN thanh heap
{	
   int  left;
...
void	Heapsort (int a[], int N)// ham heapsort
{
   int   right;
   CreateHeap(a, N); 
   right = N; 
...
void Quicksort(int a[],int l,int r) // sx phan hoach
{
	int i,j;
	int x;
	x=a[(l+r)/2];
...
//sx trôn 
int min(int a,int b)//Ðây là mot ham tro giup tra ve gia tri nho nhat cua hai so nguyên a va b.
{
return a<b?a:b;
}
...
void mergesort(int a[], int n)// sap xep hop nhat Ðây là hàm chính thuc hien sap xep tron. Nó nhan mot mang a[] và kich thuoc cua no n lam dau vao va sap xep mang theo thu tu không giam.
{
